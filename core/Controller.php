...
abstract class Controller
{
    ...
    {
        // Controllerは10文字なので、-10と指定して取り除く。
    ...
    {
        if (!preg_match('#https?://#', $url)) { // #はデリミタ https or http
        ...
        $this->protocol->setHttpHeader('Location', $url);
    }

    protected function generateCsrToken($form_name)
    {
        $key = 'csr_tokens/' . $form_name;
    ...
    protected function checkCsrfToken($form_name, $token)
    {
        ...
        $tokens = $this->session->get($key, $token);
